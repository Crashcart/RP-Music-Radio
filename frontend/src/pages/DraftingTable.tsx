...
export function DraftingTable({ drafts, onRefresh }: Props) {
  ...
  const selectAll = () => {
    const committable = drafts.filter(
        ...
        <div className="data-table-wrapper">
          ...
          <div className="draft-cards-mobile">
            {drafts.map((draft) => (
              <div
                key={draft.id}
                className={`draft-card-mobile ${selected.has(draft.id) ? "selected" : ""}`}
              >
                <div className="draft-card-top">
                  {(draft.status === "draft" ||
                    draft.status === "fleshed_out") && (
                    <input
                      type="checkbox"
                ...
                <div className="draft-card-meta">
                  {draft.genre && (
                    <span className="meta-tag">{draft.genre}</span>
                  )}
                  {draft.mood && <span className="meta-tag">{draft.mood}</span>}
                  {draft.items && (
                    <span className="meta-tag">
                      {draft.items.split("|").length} items
                    </span>
                  )}
                </div>
                {draft.script && (
                  ...
                  {draft.status !== "generating" && (
                    <button
                      ...
                      onClick={() =>
                        handleDelete(
                          draft.id,
                          draft.artist_name || draft.station_name,
                        )
                      }
                    >
                      Delete
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            >
              <div className="form-group">
                <label className="form-label" htmlFor={`station-name-${i}`}>
                  Station Name *
                </label>
                <input
                  id={`station-name-${i}`}
                  ...
                  onChange={(e) => updateRow(i, "station_name", e.target.value)}
                  autoComplete="off"
                />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor={`artist-name-${i}`}>
                  Artist / DJ *
                </label>
                <input
                  id={`artist-name-${i}`}
                  name={`artist_name_${i}`}
                  aria-label="Artist / DJ"
                  className="input"
                  placeholder="Vance Rikard"
                  value={row.artist_name}
                  onChange={(e) => updateRow(i, "artist_name", e.target.value)}
                  autoComplete="off"
                />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor={`genre-${i}`}>
                  Genre
                </label>
                <input
                  id={`genre-${i}`}
              ...
              <div className="form-group">
                <label className="form-label" htmlFor={`mood-${i}`}>
                  Mood
                </label>
                <input
                  id={`mood-${i}`}
                  ...
                  value={row.mood}
                  onChange={(e) => updateRow(i, "mood", e.target.value)}
                  autoComplete="off"
                />
              </div>
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor={`items-${i}`}>
              ...
              <input
                ...
                aria-label="Items"
                className="input"
                placeholder="Fusion Core|Med-Kit|Ammo"
                value={row.items}
        ...
        <div className="modal-actions">
          ...
          >
            {loading
              ? "Ingesting..."
              : `Ingest ${rows.filter((r) => r.station_name && r.artist_name).length} Seed(s)`}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        >
          <div className="form-group">
            <label className="form-label" htmlFor="edit-station">
              Station
            </label>
            <input
              id="edit-station"
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="edit-artist">
              Artist / DJ
            </label>
            <input
              id="edit-artist"
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="edit-mood">
              Mood
            </label>
            <input
              id="edit-mood"
        ...
        <div className="form-group">
          <label className="form-label" htmlFor="edit-items">
            Items
          </label>
          <input
            id="edit-items"
        ...
        <div className="form-group">
          <label className="form-label" htmlFor="edit-script">
            Script
          </label>
          <textarea
            id="edit-script"
            ...
            placeholder="AI-generated or hand-written DJ script..."
          />
        </div>
        <div className="form-group">
          <label className="form-label" htmlFor="edit-backstory">
            Backstory
          </label>
          <textarea
            id="edit-backstory"
        ...
        <div className="form-group">
          <label className="form-label" htmlFor="edit-market-research">
            Market Research
          </label>
          <textarea
            id="edit-market-research"
            ...
            onChange={(e) =>
              setForm({ ...form, market_research: e.target.value })
            }
            placeholder="In-universe ad copy..."
          />
        </div>
