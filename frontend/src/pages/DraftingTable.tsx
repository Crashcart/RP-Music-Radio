        ...
        {rows.map((row, i) => (
          <div key={i} style={{ marginBottom: 'var(--space-lg)', paddingBottom: 'var(--space-md)', borderBottom: i < rows.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
            ...
            <div className="form-group">
              ...
              <input id={`items-${i}`} name={`items_${i}`} aria-label="Items (pipe-separated)" className="input" placeholder="Fusion Core|Med-Kit|Ammo" value={row.items} onChange={e => updateRow(i, 'items', e.target.value)} autoComplete="off" />
            </div>
          </div>
        ))}
