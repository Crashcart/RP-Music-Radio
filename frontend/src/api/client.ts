...
export const api = {
  ...
  listStations: () => request<Station[]>("/api/v1/stations"),
  ...
  deleteStation: (id: string) =>
    request<{ deleted: string }>(`/api/v1/stations/${id}`, {
      method: "DELETE",
    }),

  generateStationArt: (id: string) =>
    request<{ art_path: string }>(`/api/v1/stations/${id}/art`, {
      method: "POST",
    }),
  ...
  listArtists: (stationId?: string) =>
    request<Artist[]>(
      `/api/v1/artists${stationId ? `?station_id=${stationId}` : ""}`,
    ),
  ...
  generatePortrait: (id: string) =>
    request<{ portrait_path: string }>(`/api/v1/artists/${id}/portrait`, {
      method: "POST",
    }),
  ...
  bulkPublish: (artistIds: string[]) =>
    request<Artist[]>("/api/v1/artists/bulk-publish", {
      method: "POST",
  ...
  bulkReject: (artistIds: string[]) =>
    request<BulkRejectResult>("/api/v1/artists/bulk-reject", {
      method: "POST",
      body: JSON.stringify({ artist_ids: artistIds }),
    }),
  ...
  bulkUndo: (artistIds: string[]) =>
    request<BulkUndoResult>("/api/v1/artists/bulk-undo", {
      method: "POST",
      body: JSON.stringify({ artist_ids: artistIds }),
    }),
  ...
  getBrand: (id: string) => request<Brand>(`/api/v1/brands/${id}`),
  ...
  ingest: (
    ...
    request<{ created: number; draft_ids: string[] }>("/api/v1/ingest", {
      method: "POST",
  ...
  listDrafts: (filters?: { status?: string; station_id?: string }) => {
    ...
    return request<{ total: number; drafts: Draft[] }>(
      `/api/v1/drafts${qs ? `?${qs}` : ""}`,
    );
  },
  ...
  commitDrafts: (draftIds: string[]) =>
    request<{
      queued: number;
      tasks: Array<{ draft_id: string; task_id: string }>;
    }>("/api/v1/commit", {
      method: "POST",
  ...
  checkApiKey: () =>
    request<{ configured: boolean; masked_key: string }>(
